...
interface ActionCardProps {
  ...
  onUpdate: (updated: ApprovalAction) => void;
}
...
export default function ActionCard({ action, onUpdate }: ActionCardProps) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      onUpdate(updated as ApprovalAction);
    } catch (err: unknown) {
  ...
  return (
    <div
      className={`bg-gray-900 border rounded-xl p-5 space-y-4 ${statusColor}`}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span
            className={`text-xs font-medium px-2 py-0.5 rounded-full ${tier.color}`}
          >
            Tier {action.tier}
          </span>
          <span className="text-lg" title={action.status}>
            {action.status_emoji}
          </span>
        </div>
      </div>
      ...
      {isPending && canAct && (
        <div className="space-y-2">
          {error && (
            <p className="text-xs text-red-400 bg-red-400/10 px-3 py-1.5 rounded">
              {error}
            </p>
          )}
          ...
          {showReject ? (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-1.5 bg-red-500 text-white text-xs font-semibold rounded-lg hover:bg-red-400 disabled:opacity-50 transition-colors"
                >
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-gray-800 text-gray-300 text-xs font-medium rounded-lg hover:bg-gray-700 transition-colors"
                >
                  Cancel
                </button>
              </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-1.5 bg-green-500/20 text-green-400 border border-green-500/30 text-xs font-semibold rounded-lg hover:bg-green-500/30 disabled:opacity-50 transition-colors"
              >
              ...
              <button
                ...
                disabled={loading !== null}
                className="flex items-center gap-1.5 px-4 py-1.5 bg-red-500/20 text-red-400 border border-red-500/30 text-xs font-semibold rounded-lg hover:bg-red-500/30 disabled:opacity-50 transition-colors"
              >
      ...
      {!isPending && (
        <div className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          {action.reviewed_by && (
            <span>by {action.reviewed_by}</span>
          )}
        </div>
      )}
