          ...
          {[
            ...
            <div
              ...
              className={`flex items-center justify-between py-2.5 border-b border-gray-800 last:border-0 ${
                user?.username === name.toLowerCase() ? "opacity-100" : "opacity-70"
              }`}
            >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs text-gray-500">
                Every action, approval, rejection, and anomaly. Immutable.
              </p>
              <button
                onClick={loadAudit}
            ...
            {!auditLoading && auditEntries.length === 0 && !auditError && (
              <p className="text-xs text-gray-500 text-center py-6">
                No audit entries yet. Queue and approve some actions.
              </p>
            )}
            ...
            {!auditLoading && auditEntries.length > 0 && (
              <div className="space-y-1.5 max-h-96 overflow-y-auto pr-1">
                {auditEntries.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex items-start gap-3 px-3 py-2 bg-gray-800/60 rounded-lg"
                  >
                    <span
                      className={`shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded mt-0.5 ${
                        EVENT_STYLES[entry.event_type] ?? "text-gray-400 bg-gray-700"
                      }`}
                    >
