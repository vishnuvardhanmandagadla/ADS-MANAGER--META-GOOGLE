        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              Pause, activate, or adjust budgets — all changes go through the
              approval queue.
            </p>
          </div>
          <button
          ...
          >
            <RefreshCw
              className={`w-4 h-4 ${loadingCampaigns ? "animate-spin" : ""}`}
            />
          </button>
        </div>
        ...
        {(loadingClients || loadingCampaigns) && (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-36 bg-gray-900 border border-gray-800 rounded-xl animate-pulse"
              />
            ))}
          </div>
        )}
        ...
        {!loadingClients && !loadingCampaigns && campaigns.length === 0 && (
          <div className="flex flex-col items-center text-center text-gray-500 py-16 bg-gray-900 border border-gray-800 rounded-xl">
            ...
            <p className="text-sm mt-1">
              No campaigns are configured for this client.
            </p>
          </div>
        )}
