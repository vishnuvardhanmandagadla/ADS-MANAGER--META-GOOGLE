...
export default function BudgetModal({ campaign, onClose, onQueued }: BudgetModalProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createAction({
        ...
        estimated_impact: delta > 0 ? `+₹${Math.abs(delta).toLocaleString()}/day additional spend` : `Save ₹${Math.abs(delta).toLocaleString()}/day`,
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <p className="text-sm text-gray-400 mb-4 truncate">{campaign.name}</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">New daily budget (INR)</label>
            ...
            {delta !== 0 && !isNaN(newBudget) && newBudget > 0 && (
              <p className={`text-xs mt-1.5 ${delta > 0 ? "text-green-400" : "text-red-400"}`}>
          ...
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="flex-1 py-2 bg-gray-800 text-gray-300 text-sm rounded-lg hover:bg-gray-700 transition-colors">
              Cancel
            </button>
            <button type="submit" disabled={!valid || loading} className="flex-1 py-2 bg-yellow-400 text-gray-950 text-sm font-semibold rounded-lg hover:bg-yellow-300 disabled:opacity-50 transition-colors">
