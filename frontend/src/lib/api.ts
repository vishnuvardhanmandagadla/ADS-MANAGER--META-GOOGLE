...
export async function getCampaigns(client_id: string): Promise<CampaignSummary[]> {
...
export async function getAllApprovals(status?: string): Promise<ApprovalsListResponse> {
  const q = status ? `?status=${status}` : "";
  return apiFetch<ApprovalsListResponse>(`/api/v1/approvals/all${q}`);
}

export async function approveAction(id: string, reviewer: string): Promise<ApprovalAction> {
...
export async function rejectAction(id: string, reviewer: string, reason: string): Promise<ApprovalAction> {
...
export async function createAction(data: CreateActionPayload): Promise<ApprovalAction> {
